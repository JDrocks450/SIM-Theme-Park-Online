...
namespace SimTheme_Park_Online.Data
{
    ...
    public class TPWCityInfo
    {
        ...
        public Structures.TPWParkResponseStructure[] GetTop10ParksStructures()
        {
            List<Structures.TPWParkResponseStructure> parks = new List<Structures.TPWParkResponseStructure>();
            foreach (var parkID in Parks)
            {
                if (!ServerManagement.Current.CityServer.TryGetParkByID(parkID, out var ParkInfo))
                    continue;
                parks.Add(ParkInfo.GetParkInfoResponse(TPWConstants.TPWServerListType.TOP10_RESULT));
            }
